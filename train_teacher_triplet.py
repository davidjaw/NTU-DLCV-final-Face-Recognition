...
def main(args):
    ...
    with tf.variable_scope('Output'):
        embed = slim.fully_connected(net_dict['PreLogitsFlatten'], 128, tf.identity, scope='Embedding')
        v_embed = slim.fully_connected(v_net_dict['PreLogitsFlatten'], 128, tf.identity, reuse=True, scope='Embedding')
        triplet_loss = utils.batch_hard_triplet_loss(train_y, embed, 1.)
        v_triplet_loss = utils.batch_hard_triplet_loss(valid_y, v_embed, 1.)
    ...
    with tf.variable_scope('Summary'):
        ...
        tf.summary.scalar('v_triplet_loss', v_triplet_loss)
    ...
    train_op = optim.minimize(train_loss + prelogits_norm + center_loss + triplet_loss, global_step)
